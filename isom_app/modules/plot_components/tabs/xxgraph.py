...
def get_edges(weights, hits_matrix, gx):
    ...
    for i in range(weights.shape[1]):
        for j in range(weights.shape[0]):
            radius = 1.5*hits_matrix[i][j]/hits_max
            #radius=.25
    ...
    for i in range(weights.shape[0]):
        for j in range(weights.shape[1]):
            ...
            for n_i in range(inters_neighs.shape[0]) : # for each neighbor n_i,
                ...
                w_i = np.round(interstitial_matrix[n_ii, n_ij], 2)*100
...
def make_graph_plot(G, Gpos, plot, node_cds, cmap):
    ...
    labels_pos = [get_midpt(Gpos, i[0],i[1]) for i in G.edges]
    ...
    graph.layout_provider = StaticLayoutProvider(graph_layout=Gpos)
    ...
    # edge_cds.selected.js_on_change('indices', CustomJS(args=dict(es=edge_cds, 
    #                                                             s2=node_cds,
    #                                                             l=labels,
    #                                                             le = labels_empty,
    #                                                             ls = labels_cds
    ...
    #         var show_labels = {'x': [], 'y': [], 'dist': []}
    #         ls.data = show_labels
    #         ls.change.emit()
            
    #         for (let i =0; i < inds.length; i++){
    #             show_labels['x'].push(es.data['mdpt_x'][inds[i]])
    #             show_labels['y'].push(es.data['mdpt_y'][inds[i]])
    #             show_labels['dist'].push(es.data['dist'][inds[i]])
    #         }
